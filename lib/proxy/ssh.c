...
int proxy_ssh_sess_init(pool *p, struct proxy_session *proxy_sess, int flags) {
#if defined(PR_USE_OPENSSL)
  int connect_policy_id, sftp_engine, proxy_role = 0, verify_server, xerrno = 0;
  config_rec *c;
  ...
  /* For now, we only implement the non-sticky connect policies. */
  connect_policy_id = proxy_reverse_get_connect_policy();
  if (proxy_reverse_policy_is_sticky(connect_policy_id) == TRUE) {
    pr_trace_msg(trace_channel, 1,
      "support for %s connect policy currently not implemented",
