...
static int validate_ecdsa_private_key(const EC_KEY *ec) {
  ...
  if (ec_privkey == NULL) {
    (void) pr_log_writefile(proxy_logfd, MOD_PROXY_VERSION,
      "error getting the EC private key: %s", proxy_ssh_crypto_get_errors());
    BN_CTX_free(bn_ctx);
    errno = EPERM;
    return -1;
  }
...
static int handle_ed25519_hostkey(pool *p, const unsigned char *key_data,
  ...
  if (agent_path == NULL) {
    ...
    ed25519_hostkey->ed25519_public_key = public_key;
    ed25519_hostkey->ed25519_public_keylen = public_keylen;
  }

  ed25519_hostkey->file_path = file_path;
  ed25519_hostkey->agent_path = agent_path;
...
static const unsigned char *ed25519_sign_data(pool *p,
  ...
  if (ed25519_hostkey->agent_path != NULL) {
     ...
     */
    buflen = bufsz = 4 + 11 + 4 + ed25519_hostkey->ed25519_public_keylen;
    ptr = buf = palloc(p, bufsz);
    ...
    return agent_sign_data(p, ed25519_hostkey->agent_path, ptr,
