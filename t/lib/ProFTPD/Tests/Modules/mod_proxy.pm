...
my $TESTS = {
  ...
  proxy_reverse_config_connect_policy_per_host_by_sql => {
    order => ++$order,
    test_class => [qw(forking mod_sql reverse)],
  },
  ...
  proxy_reverse_config_connect_policy_per_user_by_sql => {
    order => ++$order,
    test_class => [qw(forking mod_sql reverse)],
  },
...
#  },
  ...
  proxy_tls_forward_noproxyauth_login => {
    order => ++$order,
...
EOC
  ...
  if ($pid) {
    eval {
      ...
      eval { $client->login("$user\@$proxy_addr", $passwd) };
      unless ($@) {
        die("Login succeeded unexpectedly");
      }
...
sub proxy_tls_forward_noproxyauth_login {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
